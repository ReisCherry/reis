...
function isResourceDepleted(type, rx, ry, wx, wy) {
  ...
  if (type === 'gold' || type === 'diamond') {
    ...
    if (state.lockResource && state.lockResource.type === type && state.lockResource.x === rx && state.lockResource.y === ry && state.attacksOnCurrentTarget > 10) {
...
function wander(radius = 200) {
  ...
  const preferredAngle = (nameHash % 8) * (Math.PI / 4);
  // Drift towards preferred direction with random noise
  const a = preferredAngle + (Math.random() - 0.5) * 1.5;
...
function isAnotherPlayerMining(gx, gy) {
  ...
  const myDist = distTo(wx, wy);
  return state.world.players.some(p => {
    if (p.id === state.player.id) return false;
    const d = Math.hypot(p.x - wx, p.y - wy);
    // If another player is already within 150px, or is closer to the node than us and within 250px
    return d < 150 || (d < myDist && d < 250);
  });
}
...
function findBestResource(type) {
  const now = Date.now();
  const nameHash = hashCode(BOT_NAME || 'bot');
  const preferredAngle = (nameHash % 8) * (Math.PI / 4); // 8 directions: N, NE, E, SE, S, SW, W, NW
  ...
  if (activeList.length > 0) {
    ...
    for (const item of activeList) {
      ...
      // Calculate directional preference based on bot's preferred angle (only for far resources)
      const angle = Math.atan2(wy - 16000, wx - 16000);
      let angleDiff = Math.abs(angle - preferredAngle);
      if (angleDiff > Math.PI) angleDiff = 2 * Math.PI - angleDiff;
      const directionPenalty = (d > 600) ? angleDiff * 3000 : 0;

      const distToSpawn = Math.hypot(wx - 16000, wy - 16000);
      const spawnPenalty = distToSpawn < 800 ? 8000 : 0;
      
      const score = d + preferenceJitter + spawnPenalty + directionPenalty;
      if (score < bestScore) {
        bestScore = score;
  ...
  if (type === 'gold') {
    staticList = chunks.golds.filter(item => {
      ...
      if (isAnotherPlayerMining(item.x, item.y)) return false;
      const activeNode = state.world.golds.find(g => g.x === item.x && g.y === item.y);
      if (activeNode && activeNode.pct !== undefined && activeNode.pct === 0) return false; // Cooldown
    ...
    staticList = chunks.diamonds.filter(item => {
      ...
      if (isAnotherPlayerMining(item.x, item.y)) return false;
      const activeNode = state.world.diamonds.find(d => d.x === item.x && d.y === item.y);
      if (activeNode && activeNode.pct !== undefined && activeNode.pct === 0) return false; // Cooldown
      if (!isResourceSafe(item.wx, item.wy, 120)) return false;
      return !(state.failedResources.has(key) && now - state.failedResources.get(key) < 300000);
    });
  ...
  // For static resources, we can target them regardless of viewport range since untouched nodes might not be in state.world.golds/diamonds
  const candidateList = staticList;
  ...
  if (candidateList.length > 0) {
    ...
    for (const item of candidateList) {
      ...
      // Calculate directional preference based on bot's preferred angle (only for far resources)
      const angle = Math.atan2(item.wy - 16000, item.wx - 16000);
      let angleDiff = Math.abs(angle - preferredAngle);
      if (angleDiff > Math.PI) angleDiff = 2 * Math.PI - angleDiff;
      const directionPenalty = (d > 600) ? angleDiff * 3000 : 0;
...
function doGold() {
  ...
  const nodePct = stateNode ? stateNode.pct : 0.0;
...
function doDiamond() {
  ...
  const hpPct = stateNode ? stateNode.pct : 0.0;

  state.currentTarget = { type: '💎 Diamond', hpPct: 1 - hpPct, wx: near.wx, wy: near.wy, _near: near };
