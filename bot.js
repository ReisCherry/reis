...
function findBestResource(type) {
  ...
  if (activeList.length > 0) {
    ...
    for (const item of activeList) {
      ...
      const score = d + preferenceJitter + spawnPenalty;
      if (score < bestScore) {
        bestScore = score;
...
function doGold() {
  ...
  if (!state.lockResource || state.lockResource.type !== 'gold') {
    if (Date.now() - (state._lastGoldSearchTime || 0) < 10000) {
    ...
    if (!state.lockResource) {
      ...
      if (state.modeList.includes('tree')) {
        ...
        if (Date.now() - (state._lastNoGoldLog || 0) > 10000) {
          log('🔍', 'Tidak ada Gold yang aman/tersedia. Menunggu 10 detik sebelum mencari lagi...');
          state._lastNoGoldLog = Date.now();
        }
        wander(150);
      }
...
function doDiamond() {
  ...
  if (!state.lockResource || state.lockResource.type !== 'diamond') {
    if (Date.now() - (state._lastDiamondSearchTime || 0) < 10000) {
      doMonster(false);
      return;
    }
