...
function isResourceDepleted(type, rx, ry, wx, wy) {
  ...
  if (type === 'gold' || type === 'diamond') {
    ...
    // Instantly detect cooldown nodes within viewport (200px) if not in server active list
    if (d <= 200 && !exists) {
      return true;
    }
...
function moveToward(tx, ty, defaultStep = 80, stopDist = 0) {
  ...
  // Human-like speed variation scaled by player's Agility multiplier
  const speedScale = state.xp.speedMult || 1.0;
  const step = (defaultStep * speedScale) * (0.8 + Math.random() * 0.4);
