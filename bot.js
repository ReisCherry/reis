...
function sendState() {
  ...
  if (chunks.loaded.has(chunkKey)) {
    if (chunks.landTiles && chunks.landTiles.has(`${tx},${ty}`)) {
      ...
      state.player.boat = true;  // On water, use boat
    }
  } else {
    state.player.boat = false; // Default to land (walk) if map chunk is still loading
  }
...
function handleMsg(raw) {
  ...
  switch (msg.t) {
    ...
    case 'math':
      ...
      console.log('   DEBUG RAW PACKET:', JSON.stringify(msg));
      console.log(`   Question: ${msg.q || msg.question || msg.text || msg.msg || 'Unknown'}`);
      console.log(`   Options : ${msg.answers || msg.options ? (msg.answers || msg.options).join(', ') : 'None'}`);
      console.log('   Stopping activities. Waiting 10s (human reaction) before disconnecting...');
      console.log('   Bot will then sleep for 8 minutes before auto-reconnect.');
      console.log('==================================================\n');
      
      // Sound a console beep to alert the user
      process.stdout.write('\x07');
      
      state._running = false;
      stopIntervals();

      // Disconnect cleanly after 10 seconds of human reaction time
      setTimeout(() => {
        if (state.ws) {
          log('🔌', 'Closing connection cleanly (mimicking human exit)...');
        ...
        // Wait 8 minutes (480,000 ms) before reconnecting
        log('⏳', 'Entering 8 minutes cooldown to clear captcha state on server...');
        setTimeout(async () => {
          log('🔄', '8 minutes captcha cooldown finished. Reconnecting...');
          state._running = true;
          let tok = null;
          try {
            tok = await doLogin();
            if (tok) {
              ...
              log('❌', 'Auto-reconnect failed: Could not get session token.');
            }
          } catch (e) {
      ...
      }, 10000 + Math.random() * 5000); // 10-15 seconds random delay
      break;
...
function isHumanPlayer(p) {
  ...
  if (nameLower.includes('guardian') || nameLower.includes('npc') || nameLower.includes('guide') || nameLower.includes('monk') || nameLower.includes('merchant') || nameLower.includes('banker')) {
    return false;
  }
  return true;
}
...
function findBestResource(type) {
  ...
  if (type === 'gold' || type === 'diamond') {
    ...
    for (const item of finalCandidates) {
      ...
      // Dispersion penalty to distribute bots to different gold nodes (even inside the local area)
      const nodeHash = hashCode(`${type}:${item.x},${item.y}`);
      const localDispersion = ((nameHash ^ nodeHash) % 6) * 300; // 0 to 1800px penalty to force different local selections
      ...
      const score = d + spawnPenalty + localDispersion + farDispersion;
      if (score < bestScore) {
        bestScore = score;
        best = { x: item.x, y: item.y, wx: item.wx, wy: item.wy, type };
      }
...
function doGold() {
  ...
  if (state.lockResource && state.lockResource.type === 'gold') {
    ...
    if (activeNode && activeNode.pct !== undefined && activeNode.pct === 0) {
      ...
      // Check if a player is close enough to mine (within 85px) and remains there
      const isPlayerStealing = isAnotherPlayerMining(state.lockResource.x, state.lockResource.y, 85);
      if (isPlayerStealing) {
        state._playerNearNodeTicks = (state._playerNearNodeTicks || 0) + 1;
      } else {
        state._playerNearNodeTicks = 0;
      }

      if (state._playerNearNodeTicks >= 3) {
        state._playerNearNodeTicks = 0;
        if (state.attacksOnCurrentTarget > 0) {
          ...
          log('⚠️', `Gangguan terdeteksi! Pemain lain mendekati emas kita di [${state.lockResource.x}, ${state.lockResource.y}] (${state._nodeDisturbances[countKey]}/3)`);
          
          if (state._nodeDisturbances[countKey] >= 3) {
            log('🚫', `Emas di [${state.lockResource.x}, ${state.lockResource.y}] diganggu 3 kali. Mencari lokasi emas baru yang jauh dari sini.`);
            state._lastDisturbedPos = { x: state.lockResource.wx, y: state.lockResource.wy, time: Date.now() };
            if (!state.cooldownResources) state.cooldownResources = new Map();
            state.cooldownResources.set(countKey, Date.now() + 600000); // 10 menit
            state._nodeDisturbances[countKey] = 0;
          }
        } else {
          log('ℹ️', `Pemain lain berada dekat emas [${state.lockResource.x}, ${state.lockResource.y}]. Mencari target lain.`);
        }
        wander(350); // Kabur/menjauh
        state.lockResource = null;
      }
    }
  }
  ...
  if (!state._walkStartTime) state._walkStartTime = Date.now();
  if (Date.now() - state._walkStartTime > 10000 && dToStand > 5) {
    log('⚠️', `Cannot reach ${key} setelah 10 detik (nyangkut pagar?). Blacklisting.`);
    state.failedResources.set(key, Date.now());
    state.lockResource = null;
    state._walkStartTime = 0;
...
function runAttackLoop() {
  ...
  attackTick();
  state._atkTimeout = setTimeout(runAttackLoop, ATK_INTERVAL);
}
