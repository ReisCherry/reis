...
function onLoot(item, qty) {
  ...
  if ((state.mode === 'tree' && item === 'wood') ||
    ...
    if (state.lockResource) {
      ...
      state.collectingLootFrom = { x: state.lockResource.x, y: state.lockResource.y, wx: state.lockResource.wx, wy: state.lockResource.wy, time: Date.now() };
    }
...
function isResourceDepleted(type, rx, ry, wx, wy) {
  ...
  if (type === 'gold' || type === 'diamond') {
    // Disabled list exists check to prevent premature release from network viewport sync issues.
    // doGold and doDiamond rely on onLoot, activeNode.pct === 0 (cooldown), and checkRealign.
    return false;
  }
  return false;
}
...
function doGold() {
  ...
  // Stand closer to the gold node center (e.g. +40px, +5px) to ensure we are within loot pickup range!
  const standX = near.wx + 40;
  const standY = near.wy + 5;
...
function attackTick() {
  ...
  // Walk directly onto the depleted resource center to guarantee loot pickup
  if (state.collectingLootFrom) {
    const lootNode = state.collectingLootFrom;
    const dToLoot = distTo(lootNode.wx, lootNode.wy);
    if (dToLoot > 5 && Date.now() - lootNode.time < 800) {
      moveToward(lootNode.wx, lootNode.wy, 80, 0);
      return; // Wait until player walks on it or timeout expires
    } else {
      state.collectingLootFrom = null; // Done collecting
    }
  }
