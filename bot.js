...
function handleMsg(raw) {
  ...
  switch (msg.t) {
    ...
    case 'math':
      ...
      console.log('   Stopping bot. Waiting 2 minutes before auto-reconnect...');
      console.log('==================================================\n');
      
      state._running = false;
      stopIntervals();
      if (state.ws) {
        state.ws.removeAllListeners('close'); // prevent close handler from triggering standard reconnect
        state.ws.close();
      }
      
      // Schedule auto-reconnect after 120 seconds (2 minutes)
      setTimeout(async () => {
        log('🔄', '2 minutes captcha pause finished. Attempting to login and reconnect...');
        state._running = true;
        let tok = null;
        try {
          tok = await doLogin();
