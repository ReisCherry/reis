...
function sendState() {
  ...
  // ── Boat detection ──
  // CRITICAL: Gold nodes are ALWAYS on land. If we have a locked resource target
  // and are close to it, we are definitively on land — force boat=false.
  // Tile detection only fires when we are actively travelling without a nearby target.
  const hasNearbyTarget = state.lockResource &&
    distTo(state.lockResource.wx, state.lockResource.wy) <= 200;

  // Standing next to a resource or stationary = always on land, never in a boat
  if (hasNearbyTarget || !isMoving) {
    state.player.boat = false;
  } else if (chunks.loaded.has(chunkKey)) {
    // Travelling: use chunk tile data to determine land vs water
    if (chunks.landTiles && chunks.landTiles.has(`${tx},${ty}`)) {
      state.player.boat = false;
    } else {
      // Check if ANY nearby tile (±1) is land before assuming water.
      const hasNearbyLand = chunks.landTiles && (
        chunks.landTiles.has(`${tx-1},${ty}`) ||
        chunks.landTiles.has(`${tx+1},${ty}`) ||
        chunks.landTiles.has(`${tx},${ty-1}`) ||
        chunks.landTiles.has(`${tx},${ty+1}`)
      );
      state.player.boat = !hasNearbyLand;
    }
  } else {
    state.player.boat = false; // Safe default
  }
