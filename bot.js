  ...
  switch (msg.t) {
    ...
    case 'world':
      ...
      if (msg.mobs) {
            ...
            if (msg.golds) {
              const newGolds = msg.golds.map(g => Array.isArray(g) ? { x: g[0], y: g[1], pct: (g[2] !== undefined && g[2] !== null) ? g[2] / 1000 : 1.0 } : g);
              const outsideViewport = state.world.golds.filter(g => distTo(g.x * 64 + 32, g.y * 64 + 32) > 300);
              state.world.golds = [...outsideViewport, ...newGolds];
            }
            if (msg.diamonds) {
              const newDiamonds = msg.diamonds.map(d => Array.isArray(d) ? { x: d[0], y: d[1], pct: (d[2] !== undefined && d[2] !== null) ? d[2] / 1000 : 1.0 } : d);
              const outsideViewport = state.world.diamonds.filter(d => distTo(d.x * 64 + 32, d.y * 64 + 32) > 300);
              state.world.diamonds = [...outsideViewport, ...newDiamonds];
            }
      ...
      if (msg.players) {
        ...
        if (state.player.id) {
          const me = state.world.players.find(p => String(p.id) === String(state.player.id));
          if (me) {
...
function moveToward(tx, ty, defaultStep = 80, stopDist = 0) {
  ...
  state.player.x      = Math.round(state.player.x + (dx / d) * moveDist + jitterX);
  state.player.y      = Math.round(state.player.y + (dy / d) * moveDist + jitterY);
  ...
  try {
    ...
    if (data && data.t) {
      ...
      for (const t of data.t) {
        ...
        // Only consider non-zero tile types as land (type 0 is water)
        if (tileData.g && tileData.g[0] !== 0) {
...
// FARMING MODES
// ─────────────────────────────────────────────
function getMonstersNear(wx, wy, radius = 120) {
  if (!state.world.mobs) return [];
  return state.world.mobs.filter(m => {
    if (m.state === 'dead') return false;
    if (!m.type) return false;
    const mType = m.type.toLowerCase();
    // Abaikan NPC bersahabat dan penjaga (guard/defender/knight/wizard/mage/gandalf)
    if (mType.includes('monk') || mType.includes('guide') || mType.includes('merchant') || 
        mType.includes('banker') || mType.includes('npc') || mType.includes('guard') || 
        mType.includes('defender') || mType.includes('knight') || mType.includes('protector') ||
        mType.includes('gandalf') || mType.includes('mage') || mType.includes('wizard')) {
      return false;
    }
    return dist(wx, wy, m.x, m.y) < radius;
  });
}

function isResourceSafe(wx, wy, safetyRadius = 350) {
  const mobs = getMonstersNear(wx, wy, safetyRadius);
  return mobs.length === 0;
}

function isAnotherPlayerMining(gx, gy, range = 80) {
  const wx = gx * 64 + 32;
  const wy = gy * 64 + 32;
  if (!state.world.players || state.world.players.length === 0) return false;
  return state.world.players.some(p => {
    if (String(p.id) === String(state.player.id)) return false;
    
    // Ignore guards / friendly NPCs that might be sent in the players array
    if (p.name && typeof p.name === 'string') {
      const pName = p.name.toLowerCase();
      if (pName.includes('guard') || pName.includes('defender') || pName.includes('knight') || 
          pName.includes('protector') || pName.includes('monk') || pName.includes('guide') || 
          pName.includes('merchant') || pName.includes('banker') || pName.includes('npc') ||
          pName.includes('gandalf') || pName.includes('mage') || pName.includes('wizard')) {
        return false;
      }
    }
...
function findBestResource(type) {
  ...
  if (type === 'gold') {
    ...
    let debugGoldReasons = [];
    const allGolds = (state.world.golds || []).filter(g => {
      const key = `gold:${g.x},${g.y}`;
      if (g.pct !== undefined && g.pct === 0) {
        debugGoldReasons.push(`[${g.x},${g.y}](cooldown)`);
        return false;
      }
      if (isAnotherPlayerMining(g.x, g.y, 80)) {
        debugGoldReasons.push(`[${g.x},${g.y}](player mining)`);
        return false;
      }
      
      const mobsNear = getMonstersNear(g.x * 64 + 32, g.y * 64 + 32, 120);
      if (mobsNear.length > 1) {
        debugGoldReasons.push(`[${g.x},${g.y}](${mobsNear.length} mobs)`);
        return false;
      }
      if (mobsNear.length === 1) {
        const mob = mobsNear[0];
        const mType = mob.type.toLowerCase();
        const isBoss = mob.boss || mType.includes('minotaur') || mType.includes('bear');
        if (isBoss) {
          debugGoldReasons.push(`[${g.x},${g.y}](boss nearby)`);
          return false;
        }
      }
      
      if (state.failedResources.has(key) && now - state.failedResources.get(key) < 300000) {
        debugGoldReasons.push(`[${g.x},${g.y}](blacklisted)`);
        return false;
      }
      return true;
    });
    ...
    if (Date.now() - (state._lastActiveListLog || 0) > 5000) {
      log('🐛', `DEBUG: total golds in view=${state.world.golds.length} | targeting=${activeList.length}`);
      if (debugGoldReasons.length > 0) {
        log('🐛', `Rejection reasons: ${debugGoldReasons.slice(0, 10).join(', ')}`);
      }
  ...
  if (type === 'gold') {
    staticList = chunks.golds.filter(item => {
      ...
      const mobsNear = getMonstersNear(item.wx, item.wy, 120);
      if (mobsNear.length > 1) return false; // Abaikan jika dikeroyok (> 1 monster)
      if (mobsNear.length === 1) {
        const mob = mobsNear[0];
        const mType = mob.type.toLowerCase();
        const isBoss = mob.boss || mType.includes('minotaur') || mType.includes('bear');
        if (isBoss) return false; // Jangan coba-coba lawan Boss!
      }
...
function doTree() {
  ...
  if (state.lockResource && state.lockResource.type === 'tree') {
    if (!isResourceSafe(state.lockResource.wx, state.lockResource.wy)) {
...
function doGold() {
  ...
  // Jika sedang bertarung membersihkan monster di dekat Gold
  if (state.lockMobId) {
    const mob = state.world.mobs.find(m => m.id === state.lockMobId && m.state !== 'dead');
    if (mob) {
      doMonster(false);
      return;
    } else {
  ...
  if (state.lockResource && state.lockResource.type === 'gold') {
    if (isResourceDepleted('gold', state.lockResource.x, state.lockResource.y, state.lockResource.wx, state.lockResource.wy)) {
      ...
      // Lepas lock jika kita MASIH DI JALAN (belum sampai) dan ada player lain mendekati/memukul Gold ini
      const dToGold = distTo(state.lockResource.wx, state.lockResource.wy);
      if (dToGold > 80) {
        if (isAnotherPlayerMining(state.lockResource.x, state.lockResource.y, 80)) {
          log('⚠️', `Gold di [${state.lockResource.x}, ${state.lockResource.y}] sudah ditempati orang lain. Melepas target.`);
          state.lockResource = null;
          return;
        }
      }
      const mobsNear = getMonstersNear(state.lockResource.wx, state.lockResource.wy, 120);
      if (mobsNear.length > 1) {
        log('⚠️', 'Terlalu banyak monster di dekat Gold! Membatalkan dan kabur...');
        state.lockResource = null;
      } else if (mobsNear.length === 1) {
        const mob = mobsNear[0];
        const mType = mob.type.toLowerCase();
        const isBoss = mob.boss || mType.includes('minotaur') || mType.includes('bear');
        if (isBoss) {
          log('⚠️', `Boss ${mob.type} muncul dekat Gold! Membatalkan dan kabur...`);
          state.lockResource = null;
        }
      }
    }
  ...
  // Bersihkan monster jika hanya ada 1 monster biasa (non-boss) yang menghalangi jalan sebelum memukul Gold (hanya jika sudah dekat)
  if (dToStand <= 80) {
    const mobsNear = getMonstersNear(near.wx, near.wy, 120);
    if (mobsNear.length === 1) {
      const mob = mobsNear[0];
      log('⚔️', `Membersihkan monster ${mob.type} dekat Gold [${near.x},${near.y}] terlebih dahulu...`);
      state.lockMobId = mob.id;
      doMonster(false);
      return;
    }
  }
...
function doDiamond() {
  ...
  if (state.lockResource && state.lockResource.type === 'diamond') {
    if (isResourceDepleted('diamond', state.lockResource.x, state.lockResource.y, state.lockResource.wx, state.lockResource.wy)) {
      ...
      // Lepas lock jika kita MASIH DI JALAN (belum sampai) dan ada player lain mendekati/memukul Diamond ini
      const dToDmd = distTo(state.lockResource.wx, state.lockResource.wy);
      if (dToDmd > 80) {
        if (isAnotherPlayerMining(state.lockResource.x, state.lockResource.y, 80)) {
          log('⚠️', `Diamond di [${state.lockResource.x}, ${state.lockResource.y}] sudah ditempati orang lain. Melepas target.`);
          state.lockResource = null;
          return;
        }
      }
...
function attackTick() {
  ...
  if (state.modeList.length > 1 && Date.now() - modeEnteredAt > MODE_TIMEOUT_MS) {
    if (state.lockResource || state.lockMobId) {
      // Tunda rotasi mode jika sedang aktif menambang atau bertarung agar tidak meninggalkan target setengah jalan
      modeEnteredAt = Date.now();
    } else {
