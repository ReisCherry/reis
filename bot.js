...
function findBestResource(type) {
  ...
  if (type === 'gold' || type === 'diamond') {
    ...
    const candidates = list.filter(item => {
      ...
      if (isAnotherPlayerMining(item.x, item.y)) {
        return false;
      }
      ...
      // Viewport check: if node is within viewport (350px), it must be active and not on cooldown.
      const d = distTo(item.wx, item.wy);
      if (d <= VIEWPORT_RANGE) {
        if (activeNode && activeNode.pct !== undefined && activeNode.pct === 0) {
          // It is explicitly present in viewport but on cooldown -> mark as cooldown
    ...
    for (const item of finalCandidates) {
      ...
      const score = d + preferenceJitter + spawnPenalty + dispersionPenalty;
      if (score < bestScore) {
        bestScore = score;
        best = { x: item.x, y: item.y, wx: item.wx, wy: item.wy, type };
      }
