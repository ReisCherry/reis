...
function doMonster(bossOnly = false) {
  ...
  if (state.player.hpPct !== undefined) {
    if (state.player.hpPct < 0.20 && !state.isRegenerating) { // Turunkan batas kabur ke 20% biar lebih berani hajar balik
...
function attackTick() {
  ...
  if (state.world.mobs && state.player.hpPct > 0) {
    const aggroMob = state.world.mobs.find(m => {
      ...
      const defRange = state.modeList.includes('monster') ? 250 : 150; // Naikkan jarak agresif biar selalu balas dendam kalau diganggu!
      return d < defRange; 
    });
