...
function sendState() {
  ...
  // Auto-detect boat status based on tile data with a 400ms transition buffer to prevent flicker
  const tx = Math.floor(state.player.x / 64);
  const ty = Math.floor(state.player.y / 64);
  const cx = Math.floor(tx / 50);
  const cy = Math.floor(ty / 50);
  if (chunks.loaded.has(`${cx},${cy}`)) {
    const isLand = chunks.landTiles && chunks.landTiles.has(`${tx},${ty}`);
    const targetBoat = !isLand;

    if (state.player.boat !== targetBoat) {
      if (!state._boatTransitionTime) {
        state._boatTransitionTime = Date.now();
      }
      // Only switch state if we have stayed on the new tile type for 400ms
      if (Date.now() - state._boatTransitionTime > 400) {
        state.player.boat = targetBoat;
        state._boatTransitionTime = 0;
        log('⛵', targetBoat ? 'Memasuki air: menggunakan boat.' : 'Mendarat: melepas boat.');
      }
    } else {
      state._boatTransitionTime = 0;
    }
  ...
  if (!isMoving && state.player.vcx !== undefined && state.player.vcy !== undefined) {
...
function handleMsg(raw) {
  ...
  switch (msg.t) {
    ...
    case 'world':
      ...
      // Initialize inventory and equipment from islandsFull packet on login
      if (msg.i && Array.isArray(msg.i)) {
        state.inventoryItems = msg.i;
      }
      if (msg.equipped && typeof msg.equipped === 'object') {
        state.equipment = msg.equipped;
        log('🛡️', `Initial equipment loaded: ${JSON.stringify(msg.equipped)}`);
      }
      break;
...
function checkEquip() {
  ...
  // Determine what weapon/tool we need for this mode (always use weapon slot for mining/fighting)
  const wantSlot = 'weapon';
...
function moveToward(tx, ty, defaultStep = 80, stopDist = 0) {
  ...
  // Calculate natural step size based on player speed multiplier (approx 15px baseline per 100ms)
  const baseStep = 15 * (state.xp.speedMult || 1.0);
  const step = baseStep * (0.9 + Math.random() * 0.2); // Add +/- 10% natural variation
  ...
  if (moveDist > 5 && !state._detourTarget) {
...
function connect(sessionToken) {
  ...
  state.ws.on('open', () => {
    ...
    // Read char slot from session.json
    let charSlot = 0;
    if (fs.existsSync(SESSION_PATH)) {
      try {
        const sessData = JSON.parse(fs.readFileSync(SESSION_PATH, 'utf8'));
        if (sessData.char !== undefined) charSlot = sessData.char;
      } catch (e) { }
    }
